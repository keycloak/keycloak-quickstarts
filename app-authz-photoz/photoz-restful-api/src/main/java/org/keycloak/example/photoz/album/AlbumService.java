...
public class AlbumService {
	...
	public Response create(Album newAlbum) {
		...
		Query queryDuplicatedAlbum = this.entityManager
				.createQuery("from Album where name = :name and userId = :userId");
		...
		if (!queryDuplicatedAlbum.getResultList().isEmpty()) {
			throw new ErrorResponse("Name [" + newAlbum.getName() + "] already taken. Choose another one.",
					Status.CONFLICT);
		}
	...
	public Response findAll() {
		return Response.ok(this.entityManager.createQuery("from Album where userId = :id")
				.setParameter("id", request.getUserPrincipal().getName()).getResultList()).build();
	}
	...
	public Response findShares() {
		List<PermissionTicketRepresentation> permissions = getAuthzClient().protection().permission().find(null, null,
		...
		for (PermissionTicketRepresentation permission : permissions) {
			...
			if (share == null) {
				share = new SharedAlbum(
						Album.class.cast(entityManager.createQuery("from Album where externalId = :externalId")
								.setParameter("externalId", permission.getResource()).getSingleResult()));
	...
	private void createProtectedResource(Album album) {
		try {
			...
			ResourceRepresentation albumResource = new ResourceRepresentation(album.getName(), scopes,
					"/album/" + album.getId(), "http://photoz.com/album");
