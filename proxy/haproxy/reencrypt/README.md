...
with optional client certificate forwarding using the [RFC 9440](https://www.rfc-editor.org/rfc/rfc9440) `Client-Cert` header.
...
- HAProxy optionally accepts a client certificate during the TLS handshake and forwards it to Keycloak
  via the `Client-Cert` HTTP header per [RFC 9440](https://www.rfc-editor.org/rfc/rfc9440).
  The certificate is encoded as an [RFC 8941](https://www.rfc-editor.org/rfc/rfc8941#section-3.3.5) byte sequence (`:base64(DER):`).
...
- **HAProxy** listens on port 8443, terminates the incoming HTTPS connections and reencrypts the requests before forwarding them to Keycloak instances.
  ...
  via the `Client-Cert` header in RFC 9440 format.
...
that accepts certificates with the distinguished name "CN=test-client". Once the client has been initialised, the script
...
For example:
```bash
../../test-client-cert-lookup.sh 127.0.0.1.nip.io ./certs/haproxy-external/cert.pem ./certs/client/key.pem ./certs/client/cert.pem 'CN=test-client'
```
...
**Forward client certificate via RFC 9440:**

```
http-request set-header Client-Cert :%[ssl_c_der,base64]: if { ssl_c_used } { ssl_c_verify 0 }
```

When a client presents a certificate during the TLS handshake, HAProxy forwards it to Keycloak via the `Client-Cert` header.
The certificate is encoded as an [RFC 8941](https://www.rfc-editor.org/rfc/rfc8941#section-3.3.5) byte sequence: the DER-encoded certificate is base64-encoded and wrapped in colons (`:base64(DER):`).
...
**Configure RFC 9440 client certificate lookup:**

```
KC_SPI_X509CERT_LOOKUP__PROVIDER: rfc9440
```

This configures Keycloak to use the `Rfc9440ClientCertificateLookup` provider for extracting client certificates from HTTP requests.
When enabled, Keycloak reads the `Client-Cert` header set by the reverse proxy and decodes the certificate
from the [RFC 8941](https://www.rfc-editor.org/rfc/rfc8941#section-3.3.5) byte sequence format.
