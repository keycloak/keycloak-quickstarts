...
```bash
./test-client-cert-lookup.sh <hostname> <client-key> <client-cert> <subject-dn>
```

For example:
```bash
./test-client-cert-lookup.sh 127.0.0.1.nip.io ./certs/client/key.pem ./certs/client/cert.pem 'CN=test-client'
```
