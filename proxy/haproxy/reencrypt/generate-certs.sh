...
mkdir -p "$CERTS_DIR/haproxy-external"
...
openssl req -x509 \
    ...
    -keyout "$CERTS_DIR/haproxy-external/key.pem" \
    -out "$CERTS_DIR/haproxy-external/cert.pem"

cat "$CERTS_DIR/haproxy-external/cert.pem" "$CERTS_DIR/haproxy-external/key.pem" > "$CERTS_DIR/haproxy-external/combined.pem"

# Not safe! Everybody in the PC can read the private key data.
chmod 644 "$CERTS_DIR/haproxy-external/key.pem" "$CERTS_DIR/haproxy-external/cert.pem"
