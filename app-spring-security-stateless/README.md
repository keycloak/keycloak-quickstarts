app-spring-security-stateless: Stateless Spring Security Application using Cookie Store
===================================================
...
The `app-spring-security-stateless` quickstart demonstrates how to write a Spring Security application where both authentication and
...
In regards to the authentication state, the application does not use HTTP Sessions (no JSESSIONID) but a specific cookie to remember users and track their sessions. This is achieved by configuring the adapter to use `"token-store": "cookie"` and also some tweaks in your Spring Security Configuration:

* Use `SessionCreationPolicy.STATELESS`
* Use `NullAuthenticatedSessionStrategy`
